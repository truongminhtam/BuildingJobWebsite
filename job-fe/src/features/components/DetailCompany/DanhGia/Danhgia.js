...
import { addbinhluan, binhluanData, updatebinhluan } from '../../../admin/Slice/binhluanSlice';
function Danhgia(props) {
    ...
    if (binhluans) {
        for (let i = 0; i < binhluans.length; i++) {
            if (binhluans[i].companyId === +props.id && binhluans[i].status === +1) {
    ...
    const onSubmit = e => {
        ...
        if (binhluan.length === 0 || binhluan.length > 10) {
            if (binhluans.find(x => x.companyId === +companyId && x.userId === +userId)) {
                var binhluanid = binhluans.find(x => x.companyId === +companyId && x.userId === +userId);
                var idsua = binhluanid.id
                dispatch(updatebinhluan({ idsua, companyId, binhluan, userId, star, status }))
                setTimeout(() => {
                    actionbinhluan();
                }, 500);
            } else {
                dispatch(addbinhluan({ companyId, binhluan, userId, star, status }))
                setTimeout(() => {
                    actionbinhluan();
                }, 500);
            }
                        ...
                        binhluanload.map(ok => (
                            <div className="mb-5" key={ok.id}>
                                ...
                                <div className="tt-user">
                                    ...
                                    <i className="fas fa-check"></i><span className="text-success font-weight-bolder"> Khách hàng đã từng tham gia</span><br />
